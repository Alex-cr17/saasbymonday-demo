...
cp .env.example .env
# Fill .env with Supabase URL and publishable key
...
- [ ] Set `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` in `.env`
...
1. Create a new Supabase project and update `DATABASE_URL` in `.env`.
...
For ongoing schema changes, use this workflow:

1. Edit schema files in `src/lib/db/schema/*`
2. Run `npm run db:generate`
3. Add or verify RLS policies in the generated SQL under `supabase/migrations/*`
4. Run `npm run db:migrate`
5. Optionally use Supabase MCP for post-migrate checks (tables, policies, verification queries)

Drizzle schema and generated migrations remain the source of truth for app schema changes.
If migration metadata is missing in a fresh clone, `npm run db:migrate` auto-bootstraps `supabase/migrations/meta/_journal.json`.
